...
class SafetyproductController extends Controller
{
    ...
    public function actionView($id)
    {
        ...
        $smarty->_smarty->assign('model', $this->loadModel($id));
        //根据ID获取的MODEL
    ...
    public function actionEditecurityprodu()
    {
        ...
        $smarty->_smarty->assign('model', $this->loadModel($_REQUEST['id']));
        //分页HTML
    ...
    public function actionUpdate($id)
    {
        ...
        $res = array('success' => false, 'message' => '请传入要修改的安全生产管理数据！');
        if (isset($_POST['Safetyproduct'])) {
            $model->attributes = $_POST['Safetyproduct'];
            if ($model->save()) {
                $res['success'] = true;
    ...
    {
        $res = array('success' => false, 'message' => '请传入要删除的安全生产管理编号！');
        if ($_REQUEST['datas']) {
            header('Content-type:json/application;charset=utf-8');
            $count = Safetyproduct::model()->deleteAll('id in(' . $_POST['datas'] . ')');
            //删除id in ids的数据
            if ($count > 0) {
                $res['success'] = true;
    ...
    public function actionSearch()
    {
        ...
        $smarty->_smarty->assign('urlparam', $urlparam);
        $smarty->_smarty->display('cadrerealistic/page/produsearch.tpl');
    }
    /**
    ...
    public function actionListsafetyproduct()
    {
        ...
        if ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest') {
            ...
            $smarty = Yii::app()->smarty;
            $smarty->_smarty->assign('show_page', $show_page);
            //分页HTML
    ...
    public function actionExportuserexcel()
    {
        ...
        //制定列与header的顺序
        $headers = array('编号', '信息来源', '单位', '问题点', '入库时间', '解决方案', '责任部门', '责任人', '完成期限', '整改情况', '检查人', '销号时间', '备注', '问题目前状况');
